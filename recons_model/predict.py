...
from utils.utils import plot_img_and_mask, plot_img_predictedmask_and_truth
...
def predict_tensorimg(net,tensor_img,device,scale_factor=1):
    ...
    with torch.no_grad():
        output = net(tensor_img) #tensor, size()=[1,n_class,H,W]
        if net.n_classes > 1:
            probs = F.softmax(output, dim=1)[0]
        else:
            probs = torch.sigmoid(output)[0]
...
if __name__ == '__main__':
    ...
    if PREDICT_SINGLE_IMAGE:
        ...
        input_folder= train.dir_img 
        truth_folder= train.dir_mask
        index=[2000,2005]
